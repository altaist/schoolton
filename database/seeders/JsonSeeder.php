...
class JSONSeeder extends Seeder
{
    ...
    {
        $this->JSONSeed($this->table, $this->modelClass, $this->relationsConfig);
    }
        ...
        try {
            ...
            foreach($relations as $name => $config) {
                ...
                if(isset($config['flush']) && $config['flush']) {
                	...
                	$this->deleteTable(isset($config['table']) ? $config['table'] : Str::plural($name) );
                }
            }
    ...
    private function getFromJSONFile($table)
    {
        ...
        $file = file_get_contents(__DIR__ . '/' . "json/" . $table . '.json');
